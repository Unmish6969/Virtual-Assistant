...
//       setTimeout(() => {
//         startRecognition();
//       }, 800); // Small delay to avoid race condition
//     };
...
function Home() {
  ...
  const handleCommand = (data) => {
    const { type, userInput, response } = data;
    let postSpeechAction = null;

    if (type === 'google-search') {
      const query = encodeURIComponent(userInput);
      postSpeechAction = () => window.open(`https://www.google.com/search?q=${query}`, '_blank');
    } else if (type === 'calculator-open') {
      postSpeechAction = () => window.open(`https://www.google.com/search?q=calculator`, '_blank');
    } else if (type === 'instagram-open') {
      postSpeechAction = () => window.open(`https://www.instagram.com/`, '_blank');
    } else if (type === 'facebook-open') {
      postSpeechAction = () => window.open(`https://www.facebook.com/`, '_blank');
    } else if (type === 'weather-show') {
      postSpeechAction = () => window.open(`https://www.google.com/search?q=weather`, '_blank');
    } else if (type === 'youtube-search' || type === 'youtube-play') {
      const query = encodeURIComponent(userInput);
      postSpeechAction = () => window.open(`https://www.youtube.com/results?search_query=${query}`, '_blank');
    }

    speak(response, postSpeechAction);
  };
